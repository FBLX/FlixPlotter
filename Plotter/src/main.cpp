...
void setCallback(cmd* c)
{
  ...
  Argument left = cmd.getArg("l");
  Argument right = cmd.getArg("r");
  Argument maxSpeedArg = cmd.getArg("s");

  String leftVal = left.getValue();
  String rightVal = right.getValue();
  String maxSpeedVal = maxSpeedArg.getValue();
  maxSpeed = maxSpeedVal.toInt();
...
void flushCallback(cmd* c)
{
  Element * temp = commandBuffer.head;
  while (temp != NULL)
  {
    Element * t = temp->next;
    delete t;
